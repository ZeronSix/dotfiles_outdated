...
music() {
	...
	echo "%{A:mpc toggle:}%{A4:mpc next:}%{A5:mpc prev:}%{B#$red}%{F#$white}%{T2} $playing %{T2}%{F}%{B}%{A}%{A}%{A}"
}
...
lemonbar -g "970x$barheight+950" -f "boxxy" -f "fontawesome-8" -f "roboto light-8" -B "#00$black" -F "#$white" 2>/dev/null | bash
